<script>
    $(document).ready(function () {
        ...
        $('#submit').click(function(){
            ...
            $.ajax({
                method: "POST",
                url: "<?php echo e(route('primary_categories.store')); ?>",
                dataType: 'json',
                data: { _token:"<?php echo e(csrf_token()); ?>",name:name,icon:icon},
                success: function(response) {
                    $('#name').val('');
                    javaAlert(response);
                    $.ajax({
                        method: "GET",
                        url: "<?php echo e(route('PrimarylastData')); ?>",
                        dataType: 'json',
                        success: function(response) {
                            let lastData = response.lastData;
                            let cat = lastData.category_id == null ? '-': lastData.category_id;
                            var editUrl = '<?php echo e(route("primary_categories.edit", ":data")); ?>';
        ...
        $(document).on("click", "#check tr td .table-delete", function() {
            if(confirm("Are you sure, you want to delete this category? Removing this category will delete product with in this category."))
            {
                ...
                var url = '<?php echo e(route("primary_categories.destroy", ":data")); ?>';
...
<?php /**PATH /home/jojayohub/public_html/resources/views/admin/scripts/primary_category.blade.php ENDPATH**/ ?>